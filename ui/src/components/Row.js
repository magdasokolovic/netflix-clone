            ...
            <div className="movie" key={index}>
              ...
              <div key={index} className="front">
                <picture className="thumbnail">
                  <source
  srcSet = {baseImageUrl + movie.image} type =
      "image/jpg" / > <img src = {fallback} alt = "Movie Bay Logo" />
      </picture>
      ...
      <p className = {"language"}>Languages available:
          {movie.languages.toString()}</p>
                </div>

      <div className = "btn_container"><div><button className = "btn">< Link
  to = {{ pathname: "/player", state: {data: movie}, }} > {" "}<Play />
       </Link>
                    </button>< button
  className = {"btn-add btn"} > {" "}<Add />< p
  className =
      {"tooltip-add"} > Add to the list</p>
                    </button>
